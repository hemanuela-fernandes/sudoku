...
void resolve_um_passo(char quadro[9][9]) {
	...
	for (quadrante_i = 0; quadrante_i < 9; quadrante_i += 3) {
		for (quadrante_j = 0; quadrante_j < 9; quadrante_j += 3) {
			...
			linha_vazia = -1, coluna_vazia = -1;  // Guardar a posição da célula vazia.
			cont = 0;  // Quantidade de números faltante.
            quebra = 0;  // Reinicia a variável de "break"
            
			// Verificar o quadrante 3x3
			for (i = quadrante_i; i < 3 && !quebra; i++) {
				for (j = quadrante_j; j < 3 && !quebra; j++) {
					valor = quadro[i][j];
					if (valor != '0') {
						presente[valor - '1'] = 1;  // Marcar o número como presente
					}
					else {
						linha_vazia = quadrante_i + i;
						coluna_vazia = quadrante_j + j;
						cont++;
