...
FILE* carregue(char quadro[9][9]) {
	...
	switch(opcao) {
		...
		case 2:
            ...
            jogo = carregue_continue_jogo(quadro, nome_arquivo);
...
FILE* carregue_continue_jogo (char quadro[9][9], char *nome_arquivo) {
	...
	if (jogo == NULL) {
        ...
        fread(&jogada, sizeof(int), 1, jogo);
        fseek(jogo, sizeof(int) + sizeof(char) * 81 * jogada, SEEK_SET);
        fread(quadro, sizeof(char), 81, jogo);
 ...
 */
void carregue_novo_jogo(char quadro[9][9], char *nome_arquivo) {
	FILE *jogo = fopen(nome_arquivo, "r");
	int i, j;
...
FILE* crie_arquivo_binario(char quadro[9][9]) {
    ...
    if (jogo == NULL) {
        ...
        fwrite(&jogada, sizeof(int), 1, jogo);
        fwrite(quadro, sizeof(char), 81, jogo);
